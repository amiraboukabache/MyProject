...
def predict_sign():
    ...
    @messages_bp.route("/status/typing", methods=["POST"])
def set_typing_status():
    ...
    if not receiver_id or not action:
        return jsonify({"error": "receiver_id et action requis"}), 400
 
    sender_id = user.get("id")
 
    # Saved in Firestore under "TypingStatus" collection
    status_data = {
        "sender_id": sender_id,
        "receiver_id": receiver_id,
        "action": action,  # "typing" | "recording" | "idle"
        "updated_at": datetime.now(timezone.utc).isoformat()
    }
 
    doc_id = f"{sender_id}_{receiver_id}"
    firestore_db.collection("TypingStatus").document(doc_id).set(status_data)
 
    return jsonify({"message": "Status updated"}), 200
 
 
@messages_bp.route("/status/typing/<sender_id>", methods=["GET"])
def get_typing_status(sender_id):
    token = get_token_from_request()
    if not token:
        return jsonify({"error": "Token manquant"}), 401
    ...
    return jsonify({"action": data.get("action", "idle")}), 200